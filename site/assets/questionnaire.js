    ...
    container.innerHTML = themes.map((theme) => `
      <section class="q-theme" data-theme="${VV.esc(theme)}">
        ...
        ${questions.filter((q) => q.theme === theme).map((q) => {
          ...
          return `<article class="q-item" id="q-${q.uid}">
            ...
            <p style="font-size:.82rem;color:var(--ink-faint);margin:0 0 .5rem">
              Scrutin n° ${q.n} — « pour » signifie voter pour ce texte / cet amendement.
              <a href="https://www.assemblee-nationale.fr/dyn/17/scrutins/${q.n}" target="_blank" rel="noopener">source officielle</a>.
              Sélection automatique : vote parmi les plus discriminants du thème (entropie ${String(q.entropie).replace(".", ",")}).
            </p>
            <div class="q-actions" role="group" aria-label="Votre position sur le scrutin ${q.n}">
              <button type="button" class="ghost" data-uid="${q.uid}" data-value="1"${pressed(1)}>Pour</button>
              <button type="button" class="ghost" data-uid="${q.uid}" data-value="-1"${pressed(-1)}>Contre</button>
              <button type="button" class="ghost" data-uid="${q.uid}" data-value="0"${pressed(0)}>Abstention</button>
              <button type="button" class="ghost" data-uid="${q.uid}" data-value="skip"${pressed("skip")}>Passer</button>
            </div>
          </article>`;
        }).join("")}
      </section>`).join("");
  ...
  function updateResults() {
    ...
    box.innerHTML = `
      ...
      ${perGroup.map((g) => `<div class="result-row">
        <span class="stance" style="border-color:${g.couleur};color:var(--ink)">${g.sigle}</span>
        <span class="bar"><i style="width:${(g.accord * 100).toFixed(1)}%;background:${g.couleur}"></i></span>
      ...
      <details class="aide"><summary>Pourquoi ${VV.esc(best.sigle)} arrive en tête ? (votes qui ont le plus pesé)</summary>
        ...
        <ul>${drivers(best, true).map((x) => `<li><a href="https://www.assemblee-nationale.fr/dyn/17/scrutins/${x.q.n}" target="_blank" rel="noopener">${VV.esc(x.q.titre)}</a> (poids ${x.w.toFixed(2)})</li>`).join("") || "<li>Aucun</li>"}</ul>
        <h3>Votes où vous êtes en désaccord</h3>
        <ul>${drivers(best, false).map((x) => `<li><a href="https://www.assemblee-nationale.fr/dyn/17/scrutins/${x.q.n}" target="_blank" rel="noopener">${VV.esc(x.q.titre)}</a> (poids ${x.w.toFixed(2)})</li>`).join("") || "<li>Aucun</li>"}</ul>
      </details>
